...
# -------- SELEÇÃO POR ROLETA --------
def selecionar_por_torneio(populacao, fitness):
    pai1 = randint(0, len(fitness) - 1)
    pai2 = randint(0, len(fitness) - 1)
    return populacao[pai1] if fitness[pai1] > fitness[pai2] else populacao[pai2]
...
def selecionar_pais(populacao, fitness):
    ...
    for count in range(0, len(pais)):
        pais[count] = selecionar_por_torneio(populacao, fitness)
    return pais
